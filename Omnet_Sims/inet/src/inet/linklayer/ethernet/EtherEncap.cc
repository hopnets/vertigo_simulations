...
void EtherEncap::processPacketFromHigherLayer(Packet *packet)
{
    ...
    EV_DETAIL << "Encapsulating higher layer packet `" << packet->str() << "' for MAC\n";
    ...
    // add send time for fabric delay
    ethHeader->setFabric_delay_time_sent_from_source(simTime());

    auto payload_info_tag = packet->removeTagIfPresent<PayloadInfoTag>();
    if (payload_info_tag != nullptr) {
        ethHeader->setPayload_length(payload_info_tag->getPayload_length());
        ethHeader->setOffset(payload_info_tag->getOffset());
        ethHeader->setTotal_length(payload_info_tag->getTotal_length());
    }
    delete payload_info_tag;

//    std::cout << "Eth layer" << endl;
//    std::cout << "payload length: " << ethHeader->getPayload_length() << endl;
//    std::cout << "payload offset: " << ethHeader->getOffset() << endl;
//    std::cout << "payload total length " << ethHeader->getTotal_length() << endl;
//    std::cout << "--------------------------------------------" << endl;
    ...
    ethHeader->setIs_bursty(is_bursty);
