...
Register_ResultFilter("queueingTimeIncast", QueueingTimeIncastFilter);

void QueueingTimeIncastFilter::receiveSignal(cResultFilter *prev, simtime_t_cref t, cObject *object, cObject *details)
{
    if (auto msg = dynamic_cast<Packet *>(object)) {
        try {
            auto& frame = msg->peekAtFront<EthernetMacHeader>();
            if (frame->getIs_bursty())
                fire(this, t, t - msg->getArrivalTime(), details);
        }
        catch (const std::exception& e) {
            std::cout << "Warning: the msg has no macheader: " << msg->str() << endl;
        }
    }
}

Register_ResultFilter("queueingTimeFlowSize", QueueingTimeFlowSizeFilter);

void QueueingTimeFlowSizeFilter::receiveSignal(cResultFilter *prev, simtime_t_cref t, cObject *object, cObject *details)
{
    if (auto msg = dynamic_cast<Packet *>(object)) {
        try {
            auto& frame = msg->peekAtFront<EthernetMacHeader>();
            fire(this, t, frame->getTotal_length().get(), details);
        }
        catch (const std::exception& e) {
            std::cout << "Warning: the msg has no macheader: " << msg->str() << endl;
        }
...
Register_ResultFilter("getReceiverAddress", GetReceiverAddressFilter);

void GetReceiverAddressFilter::receiveSignal(cResultFilter *prev, simtime_t_cref t, cObject *object, cObject *details)
{
    EV << "SEPEHR: GetReceiverAddressFilter: Drop happened." << endl;
    long res;
    if (auto detail = dynamic_cast<PacketDropDetails*>(details)){
        if (detail->getReason() == QUEUE_OVERFLOW) {
            if ( auto packet = dynamic_cast<Packet*>(object)){

                const auto& packetDup = packet->dup();
                std::string protocol = packetDup->getName();
                if (protocol.find("arp") == std::string::npos) {
                    packetDup->setFrontIteratorPosition(b(112));
                    EV << "SEPEHR: packet dup is: " << packetDup << endl;
                    const auto& ipv4Header = packetDup->peekAtFront<Ipv4Header>();
                    std::string remoteAddrST = ipv4Header->getDestinationAddress().str();
                    EV << "SEPEHR: Remote address is: " << remoteAddrST << endl;
                    std::stringstream address(remoteAddrST);
                    std::string item;
                    int bytes[4];
                    int i = 0;
                    while (std::getline(address, item, '.'))
                    {
                        bytes[i] = std::stoi(item);
                        i++;
                    }
                    res = bytes[2]*256+bytes[3]%256;
                }
                else {
                    res = -5;
                }
                delete packetDup;
                fire(this, t, res, details);
            }
        }
    }
}

Register_ResultFilter("getSenderAddress", GetSenderAddressFilter);

void GetSenderAddressFilter::receiveSignal(cResultFilter *prev, simtime_t_cref t, cObject *object, cObject *details)
{
    EV << "SEPEHR: GetSenderAddressFilter: Drop happened." << endl;
    long res;
    if (auto detail = dynamic_cast<PacketDropDetails*>(details)){
        if (detail->getReason() == QUEUE_OVERFLOW) {
            if ( auto packet = dynamic_cast<Packet*>(object)){
                ...
                if (protocol.find("arp") == std::string::npos) {
                    ...
                    EV << "SEPEHR: packet dup is: " << packetDup << endl;
                    const auto& ipv4Header = packetDup->peekAtFront<Ipv4Header>();
                    std::string remoteAddrST = ipv4Header->getSourceAddress().str();
                    EV << "SEPEHR: Remote address is: " << remoteAddrST << endl;
                    std::stringstream address(remoteAddrST);
                    std::string item;
                    int bytes[4];
...
Register_ResultFilter("isDropForBursty", IsDropForBurstyFilter);

void IsDropForBurstyFilter::receiveSignal(cResultFilter *prev, simtime_t_cref t, cObject *object, cObject *details)
{
    EV << "SEPEHR: IsDropForBurstyFilter: Drop happened." << endl;
    long res;
    if (auto detail = dynamic_cast<PacketDropDetails*>(details)){
        if (detail->getReason() == QUEUE_OVERFLOW) {
            if ( auto packet = dynamic_cast<Packet*>(object)){
                auto frame = packet->peekAtFront<EthernetMacHeader>();
                res = long(frame->getIs_bursty());
                fire(this, t, res, details);
            }
        }
    }
...
Register_ResultFilter("getSenderAddressWithoutCheckingReason", GetSenderAddressWithoutCheckingReasonFilter);

void GetSenderAddressWithoutCheckingReasonFilter::receiveSignal(cResultFilter *prev, simtime_t_cref t, cObject *object, cObject *details)
{
    long res;
    if ( auto packet = dynamic_cast<Packet*>(object)){
        const auto& packetDup = packet->dup();
