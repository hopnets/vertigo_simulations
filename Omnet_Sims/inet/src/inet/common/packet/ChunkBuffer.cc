    ...
    if (!regions.empty()) {
        ...
        for (auto it = lowerit; it < upperit; it++) {
            ...
            if (newRegion.getEndOffset() <= oldRegion.getStartOffset() || oldRegion.getEndOffset() <= newRegion.getStartOffset())
                ...
                // removing old
                if (newRegion.creation_time > oldRegion.creation_time) {
                    // keep its creation time
                    newRegion.creation_time = oldRegion.creation_time;
                }
                ...
                previousRegion.creation_time = oldRegion.creation_time;
                nextRegion.creation_time = oldRegion.creation_time;
                oldRegion.offset = nextRegion.offset;
                oldRegion.data = nextRegion.data;
                it = regions.insert(it, previousRegion);
                return;
            }
...
void ChunkBuffer::mergeRegions(Region& previousRegion, Region& nextRegion)
{
    ...
    if (previousRegion.getEndOffset() == nextRegion.getStartOffset()) {
        // consecutive regions
        // set both creation times to min, one would be eliminated :)
        simtime_t minimum_creation_time = std::min(previousRegion.creation_time, nextRegion.creation_time);
        nextRegion.creation_time = minimum_creation_time;
        previousRegion.creation_time = minimum_creation_time;
...
void ChunkBuffer::replace(b offset, const Ptr<const Chunk>& chunk)
{
    ...
    newRegion.creation_time = now;
    sliceRegions(newRegion);
    EV << "regions size is " << regions.size() << endl;
    if (regions.empty()) {
    ...
    EV << "regions[0].creation_time: " << regions[0].creation_time << endl;
    if (regions.size() > 0) {
        if (regions[0].creation_time == 0)
            throw cRuntimeError("Something is still wrong!");
    }
    EV << "regions size is " << regions.size() << endl;
}
