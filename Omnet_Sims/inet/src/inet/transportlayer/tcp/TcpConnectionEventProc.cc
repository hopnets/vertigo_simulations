...
void TcpConnection::process_OPEN_ACTIVE(TcpEventCode& event, TcpCommand *tcpCommand, cMessage *msg)
{
    ...
    int num_mice_background_flow_apps_in_each_server, app_idx;
    std::string msg_str, delimiter, token, app_idx_str;
    switch (fsm.getState()) {
        case TCP_S_INIT:
            ...
            if (!is_bursty_indicated) {
                num_mice_background_flow_apps_in_each_server =
                            getAncestorPar("num_mice_background_flow_apps_in_each_server");
                msg_str = msg->str();
                delimiter = ".app[";
                msg_str.erase(0, msg_str.find(delimiter) + delimiter.length());
                delimiter = "]";
                app_idx_str = msg_str.substr(0, msg_str.find(delimiter));
                app_idx = std::stoi(app_idx_str);
                if (app_idx > num_mice_background_flow_apps_in_each_server){
                    // app is bursty
                    is_bursty = true;
                } else {
                    is_bursty = false;
...
void TcpConnection::process_SEND(TcpEventCode& event, TcpCommand *tcpCommand, cMessage *msg)
{
    ...
    EV_INFO << "SEPEHR: process_SEND is called with event = " << event << ", tcpCommand = " << tcpCommand << " and msg = " << msg << endl;
    EV << "SEPEHR: fsm.getState() is: " << fsm.getState() << endl;
    Packet *packet = check_and_cast<Packet *>(msg);
    EV << "SEPEHR: check_and_cast packet: " << packet << endl;
    if (!is_bursty_indicated) {
        try{
            auto main_chunk = packet->peekAtFront<GenericAppMsg>();
            if (main_chunk->getIs_micro_burst_flow()) {
                is_bursty = true;
            } else {
                is_bursty = false;
            }
            ...
            is_bursty = false;
            throw cRuntimeError("This error is actually indicating that you're not using genericAppMsg, if that's what you want, just comment this error out!");
        }
        is_bursty_indicated = true;
    }

    EV << "SEPEHR: packet length is: " << packet->getByteLength() << endl;
    switch (fsm.getState()) {
        case TCP_S_INIT:
