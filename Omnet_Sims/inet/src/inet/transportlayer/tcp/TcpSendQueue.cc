...
void TcpSendQueue::enqueueAppData(Packet *msg)
{
    ...
    EV << "SEPEHR: message received to be enqueued in tcp layer. Id is: " << msg->getId() << endl;
    EV << "SEPEHR: message received to be enqueued in tcp layer. message data length: " << msg->getDataLength() << endl;
...
Packet *TcpSendQueue::createSegmentWithBytes(uint32 fromSeq, ulong numBytes)
{
    ...
    EV << "SEPEHR: creating payload at createSegmentWithBytes" << endl;
...
//    EV << B(fromSeq - begin) << endl;
//    EV << B(numBytes) << endl;
//    EV << dataBuffer.getLength() << endl;
//    EV << dataBuffer.str() << endl;

    const auto& payload = dataBuffer.peekAt(B(fromSeq - begin), B(numBytes));   //get data from buffer
    EV << "Payload is " << payload->str() << " and packet is " << packet->str() << endl;
