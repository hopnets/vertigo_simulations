...
class INET_API AugmentedEtherMac : public EtherMacBase
{
  public:
    ...
    /*
     * Checks if a specific queue is full. Is required to support operations for valinor/DIBS
     */
    bool is_queue_full(b packet_length, std::string queue_path="");
    /*
     * This function keeps the record of the packets that are sent by the relay unit but are not
     * yet placed in the queue. We should consider them while deflecting or forwarding the packets
     * for operatins like filtering out full buffers for DIBS
     */
    void add_on_the_way_packet(b packet_length);

    /*
     * Gets the queue occupancy
     */
  ...
  protected:
    ...
    /*
     * Checks if the values of on the way packets are reasonable
     */
